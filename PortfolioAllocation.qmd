---
title: "Alocação de carteiras de investimentos baseadas em risco"
...
- Suponha que temos 2 ativos, $A$ e $B$, com retornos esperados de $3$ e $5$ e riscos associados de $2$ e $4$, respectivamente. 
- Como alocaria seus recursos de forma a maximizar seu retorno esperado e minimizar seu risco?
...
Seja um portfolio com $N$ ativos com matriz de covariância $\Sigma$.  Queremos minimizar $$L(\boldsymbol{\omega}, \lambda) = \boldsymbol{\omega}' \Sigma \boldsymbol{\omega} + \lambda (\boldsymbol{1}'\boldsymbol{\omega} - 1),$$ em que $\boldsymbol{1} = [1, 1, \cdots, 1]'.$


. . . 


Derivando _w.r.t._  $\boldsymbol{\omega}$ e $\lambda$ e igualando a zero

- $\dfrac{\partial L}{\partial \boldsymbol{\omega}} = 2 \Sigma \boldsymbol{\omega} + \lambda \boldsymbol{1} = 0$
- $\dfrac{\partial L}{\partial \lambda} = \boldsymbol{1}' \boldsymbol{\omega} - 1 = 0$

. . . 

Assim, $$\Sigma \boldsymbol{\omega} = - \dfrac{\lambda \boldsymbol{1}}{2} \rightarrow \boldsymbol{\omega} = \dfrac{-\lambda}{2} \Sigma^{-1} \boldsymbol{1}$$
...
Proposto por [Leote De Carvalho et al. (2012)](https://www.pm-research.com/content/iijpormgmt/38/3/56), os pesos são obtidos como:
...
A ideia é que cada um dos ativos tenham a mesma contribuição ao risco (volatilidade) da carteira, ou seja, que a proporção da contribuição ao risco (volatilidade) de cada ativo deve ser $1/N$.

. . . 

Os pesos são obtidos minimizando $$\displaystyle \sum_{i = 1}^N (\%RC_i - 1/N)^2,$$ sujeito á restrição de que $\boldsymbol{1}' \boldsymbol{\omega} = 1$, em que $\%RC_i = \omega_i [\Sigma \boldsymbol{\omega}]_i / \boldsymbol{\omega}' \Sigma \boldsymbol{\omega}$
...
Proposto por [Christoffersen et al. (2012)](https://academic.oup.com/rfs/article-abstract/25/12/3711/1594463?login=false), esta estratégia pode ser vista como um caso particular do portfolio de variância mínima mas com $\rho$ em lugar de $\Sigma$
